...
class Util {
  ...
  static createProgram(gl, v_shader_src, f_shader_src) {
    let vShader = Util.createShader(gl, gl.VERTEX_SHADER, v_shader_src);
    let fShader = Util.createShader(gl, gl.FRAGMENT_SHADER, f_shader_src);
...
class GLWrapper {
  ...
  draw(data, scanBarPos, scanBarLength, cursorX, cursorY) {
    ...
    drawingData = drawingData.concat(rectData).concat([cursorY, cursorX]);
    this.gl.bufferData(
...
export class IndicatorRenderer {
  ...
  init() {
    if (!this.START) {
      this.drawingData = [];
      this.gl.setUniform(this.gl.maxValLoc, this.context.maxVal);
      this.gl.setUniform(this.gl.minValLoc, this.context.minVal);
  ...
  draw(buffer) {
    ...
    if (points.length > 0) {
      if (
        ...
        this.scanBarPos =
          (this.scanBarPos + points.length) % this.context.WINDOW_POINTS;
      } else {
        for (let p of points) {
...
export class ZoomHandler {
  ...
  drawDashedLines() {
    ...
    const a_posStart = this.gl.getAttribLocation(program, "a_posStart");
    const a_lineSofar = this.gl.getAttribLocation(program, "a_lineSofar");
    ...
    const maxValLoc = this.gl.getUniformLocation(program, "maxVal");
    const minValLoc = this.gl.getUniformLocation(program, "minVal");
    ...
    for (let i = 0; i < yPos.length; i++) {
      ...
      lineSoFar += Math.sqrt(this.gl.canvas.width * this.gl.canvas.width, 0);
      positions.push(this.gl.canvas.width, yPos[i], lineSoFar, 0, yPos[i]);
      ...
      this.gl.drawArrays(this.gl.LINES, 0, positions.length / 5);
    }
    ...
    var ctx = document.getElementById("zoom-canvas-yaxis").getContext("2d");
    ctx.font = "20px Arial";
  ...
  drawGrids() {
    ...
    var gridPointLoc = this.gl.getAttribLocation(program, "gridPoint");
    ...
    this.gl.drawArrays(this.gl.LINES, 0, anotherGridPoints.length / 2);
  }
  ...
  drawLine() {
    ...
    var numPointsLoc = this.gl.getUniformLocation(program, "numPoints");
  ...
  render() {
    ...
    this.maxVal = this.data.reduce((a, b) => Math.max(a, b), -Infinity);
    this.minVal = this.data.reduce((a, b) => Math.min(a, b), Infinity);
    this.drawGrids();
    this.drawLine();
  ...
  cleanup() {
    ...
    //   .removeChild(document.getElementById("zoom-canvas"));
    // document
    //   .getElementById("model-content")
    //   .removeChild(document.getElementById("zoom-detail"));
    // document
    //   .getElementById("model-content")
    //   .removeChild(document.getElementById("zoom-canvas-yaxis"));
  }
}
